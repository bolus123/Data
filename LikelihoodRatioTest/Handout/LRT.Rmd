...
This handout will show various ways to conduct the Likelihood ratio test in practice. Suppose we are interested in the following hypothesis testing for the mean of sepal width of Virginica (from [the Iris data set](https://archive.ics.uci.edu/ml/datasets/iris)
 (@fisher1936use)) $H_0:\mu = \mu_0 = 3$ vs. $H_1: \mu = \mu_1 \ne 3$ under the assumption that the observations $X_i$'s are i.i.d. the normal distribution with $N(\mu, \sigma^2 = 0.1040)$. Also, let the significance level be $\alpha = 0.05$. First of all, I am going to describe the sepal width of Virginica.
...
Suppose the analytical form of $-2\Lambda$ is too complicated and we do not have enough sample size to reach the asymptote but we can calculate the likelihoods. We can do our test by simulating the empirical distribution of the log-likelihood ratio $-2\Lambda$.
...
iv. Calculate and save $-2\Lambda_s$ for the empirical distribution of $-2\Lambda$
v. repeat (1) - (4) until the process reaches the maximum of simulations
vi. Compare the $-2\Lambda$ calculated by the original data with the empirical distribution of $-2\Lambda$
